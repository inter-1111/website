...
// ============ Cart helpers usable across pages ============
...
// ============ Fashion Assistant (persistent sidebar) ============
// A real multi-page site means every navigation is a full page load, so the
// conversation and open/minimized state are persisted in sessionStorage and
// replayed on init() instead of always starting fresh — that's what makes
// the sidebar "carry on with the same context" across pages.
const AIAssistant = {
  history: [],
  ...
  init() {
    ...
    // The trigger lives in the header partial (desktop icon row + mobile
    // drawer), injected before init() runs — wire both here.
    ...
    // Replay whatever's already in history (from a previous page) instead
    // of resetting to the welcome message every time.
  ...
  renderWelcome() {
    this.appendAssistant("Hi! I'm your Fashion Assistant — ask me for styling advice or help finding something in the shop.", [
      ...
      { type: 'quick', label: 'Track an order', prompt: 'Track my latest order' },
    ], [], [], false);
  },
  replayHistory() {
  ...
  async send() {
    ...
    try {
      // Send recent history too, so the backend/LLM has conversational
      // context (e.g. "do you have it in blue?" referring to the last
      // product shown), not just the single latest message.
...
// ============ Dashboard sidebar nav (shared across account pages) ============
