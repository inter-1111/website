...
// ============================================================
// Optional LLM backing. Off by default — configure it either of two ways:
//
// 1) A config file (easiest, works the same on Windows/Mac/Linux):
//    copy assistant.config.example.json -> assistant.config.json in the
//    project root and fill in your url/key/model.
//
// 2) Environment variables (ASSISTANT_API_URL / ASSISTANT_API_KEY /
//    ASSISTANT_MODEL) pointing at any OpenAI-compatible /chat/completions
//    endpoint — these take priority over the config file if both are set.
//
// We deliberately do NOT ship a hardcoded key here: a key baked into
// server code that ships to users is still a leak risk, and free-tier
// keys get rate-limited or revoked without warning. Whenever no key is
// configured, or the call fails/times out/returns bad JSON, this falls
// straight back to the local rule-based engine below — so the assistant
// always works, LLM or not. Every fallback is logged to the server
// console with the reason, so it's never a silent mystery.
// ============================================================
...
// ---------- Controlled "tools" the AI may call ----------
...
function catalogSummary(limit = 250) {
  ...
  `).all(limit);
  return rows.map(r => ({ id: r.id, name: r.name, brand: r.brand, category: r.category_slug, price: r.discount_price || r.price }));
}
...
Rules:
- "reply" is warm and helpful. Pure styling questions can run a bit longer than product-search replies (a few sentences), since real advice needs room.
- "product_ids" is up to 4 ids taken EXACTLY from the catalog's "id" field, only when genuinely relevant. Pure styling-advice questions with no matching product need can leave this empty.
...
RECENT ORDERS: ${JSON.stringify(ordersSummary)}`;
  ...
  // Fold the sidebar's persisted history into real multi-turn messages, so
  // the model actually sees the conversation instead of just the latest
  // line — this is what lets it resolve "that one" / "cheaper than that"
  // across turns (and across page navigations, since the sidebar persists
  // history client-side and resends recent turns with every request).
  ...
  try {
    ...
    try {
      ...
      // Some models wrap JSON in prose despite instructions — try to salvage the first {...} block.
    ...
    // Ground product_ids against the real DB — never trust ids the model invents.
    ...
    return null; // caller falls back to the local engine
  }
}

// ============================================================
// Local rule-based fallback (also the only path when no LLM is configured)
// ============================================================
...
function extractCategoryWord(text) {
  const lower = text.toLowerCase();
  // Matches against M3RAFSH's modest-wear taxonomy (see db.js seedCategories).
  const ORDERED = [
...
// Occasion-based styling advice the local engine can offer even with no
// LLM configured — templated but genuinely useful modest-wear guidance,
// paired with a matching category so the shopper can go straight to
// relevant products.
...
function localAssistant(message, context, user) {
  ...
  if (occasionGuide) {
    ...
    products = tool_search_products({ category: occasionGuide.category, color, maxPrice, minPrice });
    if (!products.length) products = tool_search_products({ category: occasionGuide.category });
